    ...
    if form.is_submitted():
        ...
        if (not director == ''):
            ...
            for fl3 in film_list3:
                film_ids3.add(fl3[0])
        if(director == '' and year == '') or (director == '' and title == '') or (title == '' and year == '') or (title == '' and director == '' and year == ''):
...
# def choose_movie():
#     film_id = 2
#     return redirect(url_for(movie_show(film_id)))
...
def movie_show(film_id):
    ...
    poster_url = result[7]
    return render_template('movie.html', title=title, description=description, poster_url=poster_url)
